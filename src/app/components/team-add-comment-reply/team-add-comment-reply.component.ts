...
@Component({
  selector: 'app-team-add-comment-reply',
  templateUrl: './team-add-comment-reply.component.html',
  styleUrls: ['./team-add-comment-reply.component.css']
})
export class TeamAddCommentReplyComponent implements OnInit {
  ...
  addReply(e:any, textarea:HTMLTextAreaElement, parent: any){
    if(e.keyCode === 13){
      this.teamService.addComment(MyDataService.myProfile.id, this.post.id, e.target.value, parent).subscribe(res=>{
        let r = res as any
        let reply: Comment
        reply =  {
          id: r.id,
